...
class download extends admin {
	...
	function __construct() {
		...
		$this->savedir = PATH_WEB."{$_M['config'][gz_adminfile]}/update/{$this->info['type']}/{$this->info['no']}/";
	}
	...
	public function doc() {
		...
		$post = array('type'=>$this->info['type'], 'no'=>$this->info['no'], 'ver'=>$app['ver'], 'cmsver'=>$_M['config']['metcms_v'], 'user_key'=>$_M['config']['gz_secret_key']);
	...
	public function checkdllistpower() {
		...
		foreach ($dllist as $key => $val) {
			if(preg_match('/^((file)|(file_.*))\/(.*)$/', $val, $match)){
				$file[] = preg_replace('/^admin\/(.*)$/', $_M['config']['gz_adminfile'].'/'.'\\1', $match[4]);
			}
		}
		$str = '';
	...
	public function checkpower() {
		global $_M;
		$updatefile = PATH_WEB.$_M['config']['gz_adminfile'].'/update/test/test.txt';
		deldir(PATH_WEB.$_M['config']['gz_adminfile'].'/update/test/');
		if ($this->checkfilepower($updatefile)) {
			...
			$post = array('type'=>$this->info['type'], 'no'=>$this->info['no'], 'ver'=>$app['ver'], 'cmsver'=>$_M['config']['metcms_v'], 'user_key'=>$_M['config']['gz_secret_key'], 'authkey'=>$authkey, 'authcode'=>$authcode);
			$getdata = $this->curl->curl_post($post);
			...
			return $this->fail_data("{$_M['config']['gz_adminfile']}/update/{$_M['word']['write_permission']}");
		}
		return $data;
	}
		...
		if ($copydir != 'notcopy') {
			...
			while (($file = readdir($resource))!== false) {
				...
				if (is_dir($copyfile.$file)) {
					if ($file == 'admin') {
						copydir($copyfile."admin", PATH_WEB.$_M['config']['gz_adminfile']);
					}else{
	...
	protected function del_dl_file() {
		global $_M;
		if($_M['config']['gz_adminfile'] && $this->info['type'] && $this->info['no']){
				//deldir(PATH_WEB.$_M['config']['gz_adminfile'].'/update/'.$this->info['type'].'/'.$this->info['no'].'/');
		}
	}
