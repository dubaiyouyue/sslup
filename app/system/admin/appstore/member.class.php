...
class member extends admin {
	...
	public function doverifica() {
		...
		$curl -> set('host', 'http://account.resonance.com.cn/');
	...
	public function dologinout() {
		global $_M;
		$query = "UPDATE {$_M['table']['config']} SET value='' WHERE name='gz_secret_key' AND lang='metinfo'";
		DB::query($query);
	...
	public function dologingo() {
		global $_M;
		if($_M['form']['key']){
			$query = "UPDATE {$_M['table']['config']} SET value='{$_M['form']['key']}' WHERE  name='gz_secret_key' and lang = 'metinfo'";
