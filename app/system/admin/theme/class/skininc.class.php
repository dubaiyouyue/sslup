...
class skininc {
	...
	public function select($val) {
		...
		if ($val['style'] == 0) {
			...
			$gz_class1 = $array['class1'];
			$gz_class2 = $array['class2'];
			$gz_class3 = $array['class3'];
			$val['inputhtm'] ="<select name='{$val[name]}_metinfo'>";
			$val['inputhtm'].="<option value=''>{$_M[word][skinerr3]}</option>";
			switch($hngy5){
				case 1:
					foreach($gz_class1 as $key=>$val2){
						if(!$val2[if_in]){
				...
				case 3:
					foreach($gz_class1 as $key=>$val2){
						$val2['cok']=0;
						if(count($gz_class2[$val2[id]])){ 
							foreach($gz_class2[$val2[id]] as $key=>$val6){
						...
						foreach($gz_class2[$val2['id']] as $key=>$val3){
							...
							foreach($gz_class3[$val3['id']] as $key=>$val4){
