...
class theme extends admin {
	...
	function __construct() {
		global $_M;
		parent::__construct();
		$this->iniclass = new skinc($_M['form']['gz_skin_user'], $_M['lang']);
	}
	
	function doindex() {
		global $_M;
		$file = $_M['form']['gz_skin_user'] ? $_M['form']['gz_skin_user'] : $_M['config']['gz_skin_user'];
		$this->checktem($file);
		$devices = 0 ;//默认电脑版
		if($_M[form][mobile])$devices = 1 ;
		$tem = DB::get_all("SELECT * FROM {$_M[table][skin_table]} WHERE devices = '{$devices}'");//获取当前选用模板
		/*清除预览*/
		DB::query("update {$_M[table][config]} SET value = '' WHERE name = 'gz_theme_preview' and lang='{$_M[lang]}'");
		$item_index = $_M[form][item_index]?$_M[form][item_index]:1;
		$iframesrc = "{$_M[url][site]}index.php?lang={$_M[lang]}&theme_preview=1";
		if($_M['form']['mobile']){
			$iframesrc = $iframesrc.'&gz_mobileok=1';
	...
	function dolidb(){
		...
		$file = $_M['form']['gz_skin_user'] ? $_M['form']['gz_skin_user'] : $_M['config']['gz_skin_user'];
		$this->checktem($file);
		$tmpincfile=PATH_WEB."templates/{$_M[form][gz_skin_user]}/metinfo.inc.php";
		if(file_exists($tmpincfile)){
			require $tmpincfile;
		}
		//列表页设置
		$m_now_time = time();
		$gz_timetype[0]=array(1=>'Y-m-d H:i:s',2=>date('Y-m-d H:i:s',$m_now_time));
		$gz_timetype[1]=array(1=>'Y-m-d',2=>date('Y-m-d',$m_now_time));
		$gz_timetype[2]=array(1=>'Y/m/d',2=>date('Y/m/d',$m_now_time));
		$gz_timetype[3]=array(1=>'Ymd',2=>date('Ymd',$m_now_time));
		$gz_timetype[4]=array(1=>'Y-m',2=>date('Y-m',$m_now_time));
		$gz_timetype[5]=array(1=>'Y/m',2=>date('Y/m',$m_now_time));
		$gz_timetype[6]=array(1=>'Ym',2=>date('Ym',$m_now_time));
		$gz_timetype[6]=array(1=>'m-d',2=>date('m-d',$m_now_time));
		$gz_timetype[7]=array(1=>'m/d',2=>date('m/d',$m_now_time));
		$gz_timetype[8]=array(1=>'md',2=>date('md',$m_now_time));
		$selecthtml ='';
		for($i=0;$i<9;$i++){
			$selecthtml.= "<option value=\"{$gz_timetype[$i][1]}\">{$gz_timetype[$i][2]}</option>";
		}
	...
	function checktem($file) {
		...
		if($out[1]){
			$auth_domian ="gz_muban_auth_".$file;
