...
class seo extends admin {
	...
	function doseoeditor(){
		global $_M;
		$configlist = array();
		$configlist[] = 'gz_hometitle';
		$configlist[] = 'gz_title_type';
		$configlist[] = 'gz_keywords';
		$configlist[] = 'gz_alt';
		$configlist[] = 'gz_atitle';
		$configlist[] = 'gz_linkname';
		$configlist[] = 'gz_foottext';
		$configlist[] = 'gz_seo';
	...
	function dourleditor() {
		global $_M;
		$configlist = array();
		$configlist[] = 'gz_pseudo';
		configsave($configlist);/*保存系统配置*/
		if($_M['form']['gz_pseudo']){
			$query = "update {$_M['table']['lang']} SET gz_webhtm = '3' where lang='{$_M['lang']}'";
			DB::query($query);
		}else{
			$query = "update {$_M['table']['lang']} SET gz_webhtm = '0' where lang='{$_M['lang']}'";
			DB::query($query);
		}
		/*生成规则文件*/
		if($_M['form']['gz_pseudo']||$_M['form']['pseudo_download']){
			if(!$_M['form']['pseudo_download']){
				$configlist = array();
				$configlist[] = 'gz_pseudo';
				configsave($configlist);/*保存系统配置*/
				if($_M['lang']==$_M['config']['gz_index_type'] && file_exists(PATH_WEB."index.htm"))@unlink(PATH_WEB."index.htm");
				if($_M['lang']==$_M['config']['gz_index_type'] && file_exists(PATH_WEB."index.html"))@unlink(PATH_WEB."index.html");
	...
	function dositemapeditor(){
		global $_M;
		$configlist = array();
		$configlist[] = 'gz_sitemap_auto';
		$configlist[] = 'gz_sitemap_not1';
		$configlist[] = 'gz_sitemap_not2';
		$configlist[] = 'gz_sitemap_lang';
		$configlist[] = 'gz_sitemap_xml';
		$configlist[] = 'gz_sitemap_txt';
		$_M['form']['gz_sitemap_not1'] = $_M['form']['gz_sitemap_not1'] ? $_M['form']['gz_sitemap_not1'] : 0;
		$_M['form']['gz_sitemap_not2'] = $_M['form']['gz_sitemap_not2'] ? $_M['form']['gz_sitemap_not2'] : 0;
		$_M['form']['gz_sitemap_xml'] = $_M['form']['gz_sitemap_xml'] ? $_M['form']['gz_sitemap_xml'] : 0;
		$_M['form']['gz_sitemap_txt'] = $_M['form']['gz_sitemap_txt'] ? $_M['form']['gz_sitemap_txt'] : 0;
		configsave($configlist);/*保存系统配置*/
		...
		if($err){
			...
			$sitemaptype = $_M['form']['gz_sitemap_xml']?'xml':($_M['form']['gz_sitemap_txt']?'txt':0);
			sitemap_robots($sitemaptype);
			if($_M['form']['gz_sitemap_xml']||$_M['form']['gz_sitemap_txt']){
				$gent = "sitemap/index.php?lang={$_M[lang]}&htmsitemap={$_M['config']['gz_member_force']}";
