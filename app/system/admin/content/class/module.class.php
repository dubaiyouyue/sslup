...
class module extends admin {
	...
	public function url($p,$module){
		global $_M;
		$gz_class = $this->column(2,$module);
		$classnow = $p['class3']?$p['class3']:($p['class2']?$p['class2']:$p['class1']);
		$url = "{$_M['url']['site']}{$gz_class[$classnow]['foldername']}/";
		switch($module){
			case 2:
	...
	function column($type = 1,$module){
		...
		if(!$this->gz_column){
			$this->gz_column = column_sorting(1);
		}
		if($type==1){
			if($module){
				return $this->gz_column[$module];
			}
			return $this->gz_column;
		}
		if($type==2){
			$gz_class = array();
			foreach($this->gz_column[$module]['class1'] as $val){
				$gz_class[$val['id']] = $val;
			}
			foreach($this->gz_column[$module]['class2'] as $val){
				$gz_class[$val['id']] = $val;
			}
			foreach($this->gz_column[$module]['class3'] as $val){
				$gz_class[$val['id']] = $val;
			}
			return $gz_class;
		}
	...
	public function form_imglist($list,$module){
		...
		foreach($imglist as $val){
			...
			if($i==1){
				...
				if($list['imgurlr']!=str_replace('thumb/', '',$list['imgurl_l']) || $_M['config']['gz_thumb_wate'] == 1){
