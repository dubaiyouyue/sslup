...
<input id="secret_key" type="hidden" value="{$_M['config']['gz_secret_key']}">
<form method="POST" class="ui-from" name="myform" action="{$_M['url']['app_api']}a=domember_modify&user_id={$_M['form']['user_id']}&user_passpay={$_M['form']['user_passpay']}&return_type=jump&admin_url={$_M['url']['site_admin']}&user_paypass_old={$_M['form']['user_paypass_old']}&user_key={$_M['config']['gz_secret_key']}" target="_self">
