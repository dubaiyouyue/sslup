...
$tmpincfile=PATH_WEB."templates/{$_M[form][gz_skin_user]}/images/css/css.inc.php";
if(file_exists($tmpincfile)){
	require_once $tmpincfile;
}
//备用字段
$infofile=PATH_WEB."templates/{$_M[form][gz_skin_user]}/otherinfo.inc.php";
...
-->
				<div class="v52fmbx">
<input name="gz_skin_css" type="hidden" value="{$_M[config][gz_skin_css]}" />
...
$val['class']=$_M['config']['gz_skin_css']==$val[1]?'now':'';
echo <<<EOT
-->
...
<h3 class="v52fmbx_hr">{$_M[word][setseotype]}</h3>
	<dl>
		<dd class="ftype_radio ftype_transverse" style="padding-left:15px;">
			<div class="fbox">
				<label><input name="gz_urlblank" type="radio" value="0" data-checked="{$_M[config][gz_urlblank]}">{$_M[word][setseodopen]}</label>
				<label><input name="gz_urlblank" type="radio" value="1">{$_M[word][setseonewopen]}</label>
				<span class="tips">
