define(function(require, exports, module) {
	...
	function iframechuli(d){
		...
		a.each(function(){
			...
			if(h&&h.indexOf('theme_preview=1')==-1&&h.indexOf('javascript:')==-1&&h.indexOf('#')==-1&&h.indexOf('.jpg')==-1&&h.indexOf('.png')==-1&&h.indexOf('.gif')==-1){
				...
				var href = mobile==1?h+z+'theme_preview=1&gz_mobileok=1':h+z+'theme_preview=1';
				$(this).attr("href",href);
			}
		});
		/*表单*/
		...
		if(index<5){
			...
			if(d.html()==''){
				...
				var url = own_form
						...
						+'&gz_skin_user='+$("input[name='gz_skin_user']").val()
				...
				d.load(url, function(t) {
					...
					if(index==5){//全部加载完成后初始化
						set.rendering(f);
						if(f){//切换模板后
							$("input[name='gz_skin_css']").val('metinfo.css');
	...
	$(".theme-mb-ow a").click(function(){
		...
		if(t.attr("data-mbqy")!=1){
			...
			$("input[name='gz_skin_user']").val(t.parents('dl').find('dt').text());
	...
	if($(".theme-right-erweima").length>0){
		...
		$.ajax({			
			url:basepath+'app/wap/wap.php?wap_dimensional_size='+d.attr("data-size")+'&gz_dimensional_logo='+d.attr("data-logo")+'&action=dimensional&lang='+lang,
			type: 'GET',
