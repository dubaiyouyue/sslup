...
class index extends admin {
	public function doindex() {
		global $_M;
		$jsrand=str_replace('.','',$_M[config][metcms_v]).$_M[config][gz_patch];
		if ($_M['config']['gz_agents_type'] >= 2) {
			$gz_admin_logo = "{$_M[url][site]}".str_replace('../', '', $_M['config']['gz_agents_logo_index']);
		...
		if ($_M['config']['gz_agents_type'] >= 2) {
			...
			$gz_agents_display = "style=\"display:none\"";
		}else{
			$gz_admin_logo = "{$_M[url][ui]}images/logo.png";
		}
