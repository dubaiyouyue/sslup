...
function get_adminnav() {
	...
	if(!$gz_wap){
		foreach ($sidebarcolumn as $key => $val) {
			if(trim($val['name']) == 'lang_adminmobile' && $_M['config']['gz_wap'] == 0){
...
function add_table($tablenames) {
	...
	foreach($list as $key=>$val){
		$tablename = $val;
		if (strpos("|{$_M['config']['gz_tablename']}|", "|{$tablename}|") === false) {
			$_M['config']['gz_tablename'] = "{$_M['config']['gz_tablename']}|{$tablename}";
			$query = "UPDATE {$_M['table']['config']} SET value = '{$_M['config']['gz_tablename']}' WHERE name='gz_tablename'";
			DB::query($query);
...
function del_table($tablenames) {
	...
	foreach($list as $key=>$val){
		$tablename = $val;
		if (strpos("|{$_M['config']['gz_tablename']}|", "|{$tablename}|") !== false) {
			$_M['config']['gz_tablename'] = trim(str_replace("|{$tablename}|", '|', "|{$_M['config']['gz_tablename']}|"), '|');
			$query = "UPDATE {$_M['table']['config']} SET value = '{$_M['config']['gz_tablename']}' WHERE name='gz_tablename'";
			DB::query($query);
...
function sitemap_robots($sitemaptype=0){
	global $_M;
	$suffix = $sitemaptype;
	$gz_weburl_de = DB::get_one("select * from {$_M['table']['config']} where name='gz_weburl' and lang='{$_M[config][gz_index_type]}'");
	$gz_weburl_de = $gz_weburl_de['value'];
	$robots=file_get_contents(PATH_WEB.'robots.txt');
	if($suffix){
		if(stripos($robots,'Sitemap: ')===false){
			$robots.="\nSitemap: {$gz_weburl_de}sitemap.{$suffix}";
		}else{
