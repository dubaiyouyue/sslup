...
class common {
	...
	protected function load_config_global() {
		...
		$_M['config']['gz_webkeys'] = trim(file_get_contents(PATH_WEB.'/config/config_safe.php'));
		$_M['config']['gz_webkeys'] = str_replace('<?php/*', '', $_M['config']['gz_webkeys']);
		$_M['config']['gz_webkeys'] = str_replace('*/?>', '', $_M['config']['gz_webkeys']);
		if(!preg_match('/^[0-9A-Za-z]{32}$/',$_M['config']['gz_webkeys'])){
			$_M['config']['gz_webkeys'] = random(32);
			file_put_contents(PATH_WEB.'/config/config_safe.php', "<?php/*{$_M['config']['gz_webkeys']}*/?>");
		}
		$_M['config']['gz_adminfile_code'] = $_M['config']['gz_adminfile'];
		$_M['config']['gz_adminfile'] = authcode($_M['config']['gz_adminfile'],'DECODE', $_M['config']['gz_webkeys']);
		
		$_M['table'] = array();
		$_Mettables = explode('|', $_M['config']['gz_tablename']);
		foreach ($_Mettables as $key => $val) {
			$_M['table'][$val] = $_M['config']['tablepre'].$val;
		}
		//$_M['config']['gz_host_new'] = 'app.resonance.com.cn';
		$_M['config']['gz_host'] = $_M['config']['gz_host_new'];		
	}			
	...
	protected function load_config_lang() {
		global $_M;
		$_M['lang'] = $_M['form']['lang'] ? $_M['form']['lang'] : $_M['config']['gz_index_type'];
	...
	protected function load_url_site() {
		global $_M;
		$_M['url']['site'] = $_M['config']['gz_weburl'];
		$_M['url']['site_admin'] = $_M['url']['site'].$_M['config']['gz_adminfile'].'/';
	}
	...
	protected function load_url_other() {
		...
		$_M['url']['api'] = 'http://'.$_M['config']['gz_host'].'/'."index.php?lang=".$_M['lang'].'&';
	}
