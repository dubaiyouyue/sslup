...
class admin extends common {
	...
	protected function load_agent_word($lang) {
		...
		if ($_M['config']['gz_agents_type'] >= 2) {
			...
			$_M['word']['indexthanks'] = $lang_agents['gz_agents_thanks'];
			$_M['word']['metinfo'] = $lang_agents['gz_agents_name'];
			$_M['word']['copyright'] = $lang_agents['gz_agents_copyright'];
			$_M['word']['oginmetinfo'] = $lang_agents['gz_agents_depict_login'];
		}
	}
	...
	protected function check() {
		...
		$gz_adminfile = $_M['config']['gz_adminfile'];
		...
		if (!$metinfo_admin_name || !$metinfo_admin_pass) {
			if ($admin_index) {
				gz_cooike_unset();
				gz_setcookie("re_url", $re_url,time()-3600);
				...
				if (!$_COOKIE[re_url]&&!strstr($re_url, "return.php")) gz_setcookie("re_url", $re_url,time()+3600);
				gz_cooike_unset();
				Header("Location: ".$_M['url']['site_admin']."login/login.php");
			}
			exit;
			...
			if (!$admincp_ok) {
				if ($admin_index) {
					...
					if (!strstr($re_url, "return.php")) {
						if (!$_COOKIE['re_url']) gz_setcookie("re_url", $re_url,time()+3600);
					}
					gz_cooike_unset();
					Header("Location: ".$_M['url']['site_admin']."login/login.php");
				}
