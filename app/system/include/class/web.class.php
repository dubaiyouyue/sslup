...
class web extends common {
	...
	protected function load_template_lang() {
		global $_M;
		//模板文件
		$query = "SELECT * FROM {$_M['table']['templates']} WHERE no='{$_M[config][gz_skin_user]}' AND lang='{$_M['lang']}' order by no_order ";
	...
	protected function check($groupid = 0) {	
		...
		$gourl = $gourl == -1 ? "":$gourl;
		if($_M['lang'] != $_M['config']['gz_index_type']){
			$lang = "&lang={$_M['lang']}";
		}
		if($groupid == 0 && !$user){
	...
	/**
	  ...
	  * 如果会员拥有权限则，程序代码向后正常执行，如果没有则提示没有权限。
	  * get_gz_cookie函数兼容也调用login_by_auth,如果修改请一并修改。
	  */
	protected function get_login_user_info($gz_auth = '', $gz_key = '') {
		global $_M;
		$gz_auth =  $gz_auth ? $gz_auth : $_M['form']['acc_auth'];
		$gz_key  =  $gz_key  ?  $gz_key : $_M['form']['acc_key'];
		$userclass = load::sys_class('user', 'new');
		if($gz_auth && $gz_key) {
			if(!$userclass->get_login_user_info()){
				$userclass->login_by_auth($gz_auth, $gz_key);
			}
		}
	...
	protected function tem_dir(){
		...
		if($ua != '' && preg_match($uachar, $ua) && $_M['config']['gz_wap']){
			$_M['config']['gz_skin_user'] = $_M['config']['wap_skin_user'];
		}else{
			$_M['config']['gz_skin_user'] = $_M['config']['gz_skin_user'];
		}
		define('PATH_TEM', PATH_WEB."templates/".$_M['config']['gz_skin_user'].'/');//模板根目录
	}
