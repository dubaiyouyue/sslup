 ...
 * @param string $gz_image_transition	水印图像和原始图像的融合度(1 = 100)
...
class watermark {
	...
	public function __construct() {
		global $_M;
		$this->set('water_image_name', $_M['config']['gz_wate_bigimg']);	
		$this->set('water_pos', $_M['config']['gz_watermark']);		
		$this->set('water_text', $_M['config']['gz_text_wate']);
		$this->set('water_text_size', $_M['config']['gz_text_bigsize']);
		$this->set('water_text_angle', $_M['config']['gz_text_angle']);
		$this->set('water_text_font', $_M['config']['gz_text_fonts']);
		$this->set('water_text_color', $_M['config']['gz_text_color']);
	}
	...
	public function set_system_watermark() {
		global $_M;
		if ($_M['config']['gz_wate_class'] == 1) {
			$this->set_textmark();
		}
		if ($_M['config']['gz_wate_class'] == 2) {
			$this->set_imgmark();
		}
	}
	...
	public function set_thumb() {
		global $_M;
		$this->set('water_image_name', $_M['config']['gz_wate_img']);
		$this->set('water_pos', $_M['config']['gz_watermark']);	
		$this->set('water_text', $_M['config']['gz_text_wate']);
		$this->set('water_text_size', $_M['config']['gz_text_size']);
		$this->set('water_text_angle', $_M['config']['gz_text_angle']);
		$this->set('water_text_font', $_M['config']['gz_text_fonts']);
		$this->set('water_text_color', $_M['config']['gz_text_color']);		
	...
	public function set_system_thumb() {
		global $_M;
		$this->set_thumb();
		$this->set('is_watermark', $_M['config']['gz_thumb_wate']);
	}
	...
	public function set_system_bigimg() {
		...
		$this->set_system_watermark();
		$this->set_bigimg();
		$this->set('is_watermark', $_M['config']['gz_big_wate']);
	}
	...
	public function set($name, $value) {
		...
		switch($name) {
			...
			case 'water_text_font':
				$this->water_text_font = str_replace(PATH_WEB.$_M['config']['gz_adminfile'].'/', '', $value);
				$this->water_text_font = PATH_WEB.$_M['config']['gz_adminfile'].'/'.str_replace('../', '', $value);
			break;
			case 'gz_image_transition':
				$this->gz_image_transition = $value;
			break;
			case 'jpeg_quality':
	...
	public function create($water_scr_image) {
		...
		if ($this->water_mark_type == 'img') {
			...
			$gz_image_type = $this->get_type($this->water_image_name);
			$gz_image = $this->createImage($gz_image_type,$this->water_image_name);
			$gz_image_w=ImageSX($gz_image);
			$gz_image_h=ImageSY($gz_image);
			$temp_gz_image = $this->getPos($src_image_w,$src_image_h,$this->water_pos,$gz_image);
			$gz_image_x = $temp_gz_image["dest_x"];
			$gz_image_y = $temp_gz_image["dest_y"];
			if ($this->get_type($this->water_image_name)=='png') {
				imagecopy($src_image, $gz_image, $gz_image_x, $gz_image_y, 0, 0, $gz_image_w, $gz_image_h);
			} else {
				imagecopymerge($src_image, $gz_image, $gz_image_x, $gz_image_y, 0, 0, $gz_image_w, $gz_image_h, $this->gz_image_transition);
			}
		}
	 ...
	 * 8 = middle left		左中
	 * @param string $gz_image:			createImage创建的图片资源
	 * @return array						返回水印位置	
	 */
	protected function getPos($sourcefile_width, $sourcefile_height, $pos, $gz_image="") {
		if ($gz_image) {
