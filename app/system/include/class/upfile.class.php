...
class upfile {
	...
	public function set_upfile() {
		...
		$this->set('format', $_M['config']['gz_file_format']);
		$this->set('maxsize', min($_M['config']['gz_file_maxsize']*1048576, 1073741824));
		$this->set('is_rename', $_M['config']['gz_img_rename']);
	}
	...
	public function upload($form = '') {
		...
		//文件大小是否正确
		if ($filear["size"] > $this->maxsize || $filear["size"] > $_M['config']['gz_file_maxsize']*1048576) {
		...
		if ($_M['config']['gz_file_format']) {
			if($_M['config']['gz_file_format'] != "" && !in_array(strtolower($this->ext), explode('|',strtolower($_M['config']['gz_file_format']))) && $filear){  	
				return $this->error($this->ext." {$_M['word']['upfileTip3']}");
			}
		} else {
			return $this->error($this->ext." {$_M['word']['upfileTip3']}");
