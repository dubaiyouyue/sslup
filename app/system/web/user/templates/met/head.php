...
$loginbg = $_M['config']['gz_member_bgimage']?" background-image:url({$_M['config']['gz_member_bgimage']});":'';
echo <<<EOT
--><!DOCTYPE HTML>
<html>
<head>
...
<meta name="generator" content="MetInfo"  data-variable="{$_M[url][site]}|{$_M[lang]}|{$classnow}|{$id}|{$class_list[$classnow][module]}|{$_M[config][gz_skin_user]}" />
...
<style>
.met-member{ background-color:{$_M['config']['gz_member_bgcolor']};}
.login_index{ background-color:{$_M['config']['gz_member_bgcolor']}; {$loginbg} }
</style>
<body>
<div class="container met-head">
	...
	<ul class="list-none">
		<li><a href="{$_M['url']['user_home']}" class="met-logo"><img src="{$_M['config']['gz_logo']}" /></a></li>
<!--
EOT;
if($title){
echo <<<EOT
