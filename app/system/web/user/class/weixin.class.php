...
class weixin extends other {
	...
	public function __construct() {
		global $_M;
		if(!$_M['config']['gz_weixin_open']){
			okinfo($_M['url']['login'], '微信登录功能已关闭');
		}
		if(is_weixin_client()){
			$this->appid = $_M['config']['gz_weixin_gz_appid'];
			$this->appkey = $_M['config']['gz_weixin_gz_appsecret'];
		}else{
			$this->appid = $_M['config']['gz_weixin_appid'];
			$this->appkey = $_M['config']['gz_weixin_appsecret'];
		}
