...
class valid {	
	...
	public function get_email($email,$type = 'register'){
		...
		$title = $_M['config']['gz_member_email_reg_title'];
		$body = $_M['config']['gz_member_email_reg_content'];
		$url = $_M['url']['valid_email'];
		
		if($type=='getpassword'){
			$title = $_M['config']['gz_member_email_password_title'];
			$body = $_M['config']['gz_member_email_password_content'];
			$url = $_M['url']['password_valid'];
		}
		if($type=='emailedit'){
			$title = $_M['config']['gz_member_email_safety_title'];
			$body = $_M['config']['gz_member_email_safety_content'];
			$url = $_M['url']['mailedit'];
		}
		if($type=='emailadd'){
			$title = $_M['config']['gz_member_email_safety_title'];
	...
	public function repalce_email($str, $url){
		global $_M;
		$str = str_replace('{webname}', $_M['config']['gz_webname'], $str);
		$str = str_replace('{weburl}', $_M['config']['gz_weburl'], $str);
	...
	public function get_tel($tel){
		...
		$ret = $sms->sendsms($tel, "验证码为 {$code} ，请及时输入验证。({$_M['config']['gz_webname']})");
		return $ret;
