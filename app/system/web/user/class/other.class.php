...
class other{
	...
	public function __construct() {
		global $_M;
		$this->appid = $_M['config']['gz_weibo_appkey'];
		$this->appkey = $_M['config']['gz_weibo_appsecret'];
	...
	public function get_user_by_openid($unionid){	
		...
		if($other_user['gz_uid']){
			$user = load::sys_class('user', 'new')->get_user_by_id($other_user['gz_uid']);
		}
		return $user;
	}
	...
	public function insert_other_user($gz_uid, $openid, $unionid, $access_token, $expires_in){
		$query = "INSERT INTO {$this->table} SET gz_uid='{$gz_uid}', openid = '{$openid}',unionid='{$unionid}',access_token='{$access_token}', expires_in='{$expires_in}', type='{$this->type}'";
		return DB::query($query);
	}
	
	public function update_other_user($unionid, $openid = '#' , $gz_uid = '#', $access_token = '#', $expires_in = '#'){
		if($gz_uid != '#'){
			$sql .= "gz_uid='{$gz_uid}',";
		}
		if($access_token != '#'){
